...
cert_status_t cert_check_existing_files(const char *client_id, const char *cert_type, cert_file_info_t *file_info) {
    ...
    // Search for certificate files matching the pattern
    if (glob(pattern_cert, GLOB_NOSORT, NULL, &glob_result) == 0 && glob_result.gl_pathc > 0) {
        // Use the most recent file (last in alphabetical order due to timestamp naming)
        const char* latest_cert = glob_result.gl_pathv[glob_result.gl_pathc - 1];
        
        // Extract timestamp from filename and create corresponding key path
        char *timestamp_start = strrchr(latest_cert, '_');
        if (timestamp_start) {
            timestamp_start++; // Skip the underscore
            char *dot = strrchr(timestamp_start, '.');
            if (dot) {
                size_t timestamp_len = dot - timestamp_start;
                char timestamp[32];
                strncpy(timestamp, timestamp_start, timestamp_len);
                timestamp[timestamp_len] = '\0';
                
                // Create the corresponding key path
                snprintf(file_info->key_path, sizeof(file_info->key_path), 
                        "%s/%s_%s_%s.key", CERT_DIRECTORY, cert_type, client_id, timestamp);
                
                // Check if both files exist
                if (cert_is_file_readable(latest_cert) && cert_is_file_readable(file_info->key_path)) {
                    strncpy(file_info->cert_path, latest_cert, sizeof(file_info->cert_path) - 1);
                    file_info->cert_path[sizeof(file_info->cert_path) - 1] = '\0';
                    file_info->exists = true;
                    file_info->creation_time = atol(timestamp);
