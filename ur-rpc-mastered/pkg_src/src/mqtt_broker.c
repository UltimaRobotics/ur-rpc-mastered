...
int mqtt_broker_init(mqtt_broker_t *broker, const char *config_file) {
    ...
    broker->notification_manager = malloc(sizeof(notification_manager_t));
    if (!broker->notification_manager) {
        LOG_ERROR("Failed to allocate notification manager");
        mqtt_broker_cleanup(broker);
        return -1;
    }

    if (notification_manager_init(broker->notification_manager, NULL) != 0) {
        LOG_ERROR("Failed to initialize notification manager");
        free(broker->notification_manager);
        broker->notification_manager = NULL;
        mqtt_broker_cleanup(broker);
        return -1;
    }
    
    // Configure notification system from broker config
    if (broker->notification_manager) {
        broker->notification_manager->config.enabled = broker->config.notification_enabled;
        if (strlen(broker->config.notification_topic) > 0) {
            strncpy(broker->notification_manager->config.default_notification_topic, 
                   broker->config.notification_topic,
                   MAX_NOTIFICATION_TOPIC_LEN - 1);
            broker->notification_manager->config.default_notification_topic[MAX_NOTIFICATION_TOPIC_LEN - 1] = '\0';
        }
        LOG_INFO("Notification system configured: enabled=%s, topic=%s, destination=%s", 
                 broker->config.notification_enabled ? "yes" : "no",
                 broker->config.notification_topic,
                 broker->config.notification_destination_client_id);
    }

    // Link notification manager to client manager
    client_manager_set_notification_manager(&broker->client_manager, broker->notification_manager);
    
    // Initialize certificate manager if enabled
    if (broker->config.cert_generation_enabled) {
        if (!cert_manager_init(broker->config.cert_ca_cert_file, 
                              broker->config.cert_ca_key_file,
                              broker->config.cert_output_directory)) {
            LOG_WARNING("Failed to initialize certificate manager - certificate generation disabled");
            broker->config.cert_generation_enabled = false;
        } else {
            LOG_INFO("Certificate manager initialized: CA=%s, output_dir=%s", 
                     broker->config.cert_ca_cert_file, 
                     broker->config.cert_output_directory);
        }
    }
...
void mqtt_broker_cleanup(mqtt_broker_t *broker) {
    ...
    if (broker->notification_manager) {
        notification_manager_cleanup(broker->notification_manager);
        free(broker->notification_manager);
        broker->notification_manager = NULL;
    }
...
int mqtt_broker_accept_client(mqtt_broker_t *broker, int listen_fd, bool use_ssl) {
    ...
    // SSL Security Feature: When SSL is enabled, reject TCP connections unless bypass flag is defined
    if (broker->config.ssl_enabled && !use_ssl) {
#ifndef BrByPASS
        char client_ip[INET6_ADDRSTRLEN];
        network_get_peer_address(client_fd, client_ip, sizeof(client_ip));
        LOG_WARNING("TCP connection from %s rejected - SSL required (fd=%d)", client_ip, client_fd);
        close(client_fd);
        return -1;
#else
        char client_ip[INET6_ADDRSTRLEN];
        network_get_peer_address(client_fd, client_ip, sizeof(client_ip));
        LOG_INFO("TCP connection from %s accepted - SSL bypass enabled (fd=%d)", client_ip, client_fd);
#endif
    }
    ...
    // For SSL clients, perform initial security validation during handshake
    if (use_ssl && client->ssl_ctx) {
        // The actual handshake and validation will happen during first data exchange
        LOG_DEBUG("SSL client initialized, handshake will occur during data exchange");
    }
...
void mqtt_broker_disconnect_client(mqtt_broker_t *broker, int client_fd) {
    ...
    if (client) {
        ...
        client_manager_handle_disconnect(&broker->client_manager, client);
