...
static void print_startup_info(mqtt_broker_t *broker) {
    ...
    if (broker->config.ssl_enabled) {
        printf("SSL Port:     %d\n", broker->config.ssl_port);
    }
    printf("Bind Address: %s\n", broker->config.bind_address);
    printf("Max Clients:  %d\n", broker->config.max_clients);
    printf("Log Level:    %d\n", broker->config.log_level);
    printf("Anonymous:    %s\n", broker->config.allow_anonymous ? "yes" : "no");
    printf("SSL/TLS:      %s\n", broker->config.ssl_enabled ? "yes" : "no");
...
static void print_shutdown_info(mqtt_broker_t *broker) {
    uint64_t uptime, active_clients, total_messages;
    mqtt_broker_get_stats(broker, &uptime, &active_clients, &total_messages);
    
    printf("\n");
    printf("=== MQTT Broker Shutdown ===\n");
    printf("Uptime:        %lu seconds\n", uptime);
    printf("Total Messages: %lu\n", total_messages);
    printf("Total Clients:  %lu\n", broker->total_clients);
